...
Span context SHALL propagate via `contextvars` from a per-invocation root span (CLI command, API request via middleware, or embeddable-core entry) through nested decorated calls, including across the per-job async tasks, with the run id as the correlation id. Span events SHALL flow to a sink selected once at startup by `OBSERVABILITY_SINK`: `json` — one structured log line per span through the shared logger, the default; `none` — fully disabled; `openobserve` — `json` plus batched delivery to OpenObserve's REST JSON-ingestion API (`{OPENOBSERVE_URL}/api/{OPENOBSERVE_ORG}/{OPENOBSERVE_STREAM}/_json`, basic auth). Telemetry delivery SHALL be fire-and-forget: an unreachable OpenObserve logs one warning and SHALL NOT fail, block, or measurably slow the run. No OpenTelemetry SDK SHALL be a dependency of this change; application code SHALL depend only on the sink protocol so an OTel layer can be inserted before OpenObserve later. With the sink disabled, run outputs SHALL be byte-identical to instrumented runs, and an eval SHALL verify span nesting matches the call tree on a fixture run.
