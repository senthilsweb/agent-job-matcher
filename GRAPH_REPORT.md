# Graph Report - /home/runner/work/agent-job-matcher/agent-job-matcher  (2026-07-20)

## Corpus Check
- 82 files · ~106,050 words
...
9. `_Strict` - 19 edges
10. `extract_resume_text()` - 19 edges
...
Cohesion: 0.07
Nodes (68): BaseModel, CandidateIdentity, Whatever contact fields were found in the resume text — never a guess., FetchResult, The outcome of the single fetch attempt for one job source., File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J, Stable core entry point — lazy import so `import job_matcher` stays light., Stable core re-export of resume extraction (lazy import). (+60 more)
...
Cohesion: 0.09
Nodes (40): analyze(), _custom_openapi(), health(), File Name: api.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Description, FastAPI-native generation plus the repo link (spec: repo path link)., One span per HTTP request — the root the pipeline's run span nests under., Run the full analysis synchronously and return the typed outcome array.      Pro, Extract the resume into the standard JSON Resume format.      Provide the resume (+32 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (32): jsonresume(), assert_contact_grounded(), Award, Basics, Certificate, _digits(), Education, extract_jsonresume() (+24 more)

### Community 5 - "Community 5"
Cohesion: 0.11
Nodes (26): _count_matched(), js_round(), match_band_for(), recommendation_for(), score_analysis(), score_job_fit(), File Name: test_match_banding.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2, test_band_boundaries() (+18 more)
...
Cohesion: 0.11
Nodes (23): extract_jsonresume(), extract_resume_text(), run_analysis(), test_full_run_cover_letter_is_grounded_in_resume_text(), File Name: test_embeddable_core.py Author: Senthilnathan Karuppaiah Date: 11-JUL, test_deterministic_pieces_standalone(), test_full_offline_run_via_package_root(), test_extract_stamps_schema_version() (+15 more)
...
- **Thin community `Community 20`** (2 nodes): `layout.tsx`, `RootLayout()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (2 nodes): `handleCopy()`, `cover-letter-section.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (2 nodes): `status-pill.tsx`, `StatusPill()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (2 nodes): `resume-dropzone.tsx`, `ResumeDropzone()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (2 nodes): `score-bar.tsx`, `ScoreBar()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (2 nodes): `cn()`, `button.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (2 nodes): `separator.tsx`, `cn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 27`** (2 nodes): `label.tsx`, `cn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 28`** (2 nodes): `Collapsible()`, `collapsible.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (2 nodes): `input.tsx`, `Input()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (2 nodes): `use-backend-status.ts`, `useBackendStatus()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 39`** (1 nodes): `report-card.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (1 nodes): `results-panel.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (1 nodes): `nav-rail.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `JobReport` connect `Community 1` to `Community 8`, `Community 2`, `Community 3`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J` connect `Community 1` to `Community 0`, `Community 7`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Are the 34 inferred relationships involving `JobReport` (e.g. with `File Name: report.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descript` and `UTC timestamp in the filename-safe contract format.`) actually correct?**
  _`JobReport` has 34 INFERRED edges - model-reasoned connections that need verification._
- **Are the 25 inferred relationships involving `JobAnalysis` (e.g. with `File Name: scoring.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descrip` and `Round half away from zero for positives — JS Math.round semantics (2.5 → 3).`) actually correct?**
...
- **Are the 17 inferred relationships involving `OpenObserveRestSink` (e.g. with `Span` and `Sink`) actually correct?**
