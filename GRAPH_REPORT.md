# Graph Report - /home/runner/work/agent-job-matcher/agent-job-matcher  (2026-08-03)

## Corpus Check
- 82 files · ~106,265 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 521 nodes · 973 edges · 49 communities detected
...
5. `GET()` - 21 edges
6. `ScoreBreakdown` - 21 edges
7. `JobFetchFailure` - 20 edges
8. `JsonLogSink` - 20 edges
9. `extract_resume_text()` - 19 edges
10. `_Strict` - 19 edges
...
Cohesion: 0.07
Nodes (57): BaseModel, extract_jsonresume(), File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J, Stable core entry point — lazy import so `import job_matcher` stays light., Stable core re-export of resume extraction (lazy import)., Stable core re-export of JSON Resume extraction (lazy import, async)., build_job_report(), File Name: report.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descript (+49 more)
...
Cohesion: 0.07
Nodes (50): analyze(), _custom_openapi(), health(), File Name: api.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Description, FastAPI-native generation plus the repo link (spec: repo path link)., One span per HTTP request — the root the pipeline's run span nests under., Run the full analysis synchronously and return the typed outcome array.      Pro, Extract the resume into the standard JSON Resume format.      Provide the resume (+42 more)
...
Cohesion: 0.18
Nodes (25): CandidateIdentity, Whatever contact fields were found in the resume text — never a guess., FetchResult, The outcome of the single fetch attempt for one job source., _cover_letter_text(), _job_task(), _now(), _persist() (+17 more)
...
Cohesion: 0.12
Nodes (21): extract_resume_text(), run_analysis(), test_full_run_cover_letter_is_grounded_in_resume_text(), File Name: test_embeddable_core.py Author: Senthilnathan Karuppaiah Date: 11-JUL, test_deterministic_pieces_standalone(), test_full_offline_run_via_package_root(), test_all_failed_run_completes_cleanly(), test_analysis_exception_isolated_to_its_job() (+13 more)
...
Cohesion: 0.18
Nodes (18): max_fetch_bytes(), _blocked_host_reason(), fetch_job_source(), _fetch_local(), html_to_text(), _now(), File Name: fetch.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descripti, Make the one and only fetch attempt for a job source. Never retries. (+10 more)
...
- **Thin community `Community 20`** (2 nodes): `layout.tsx`, `RootLayout()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 21`** (2 nodes): `main()`, `export-openapi.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 22`** (2 nodes): `utils.ts`, `cn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (2 nodes): `use-backend-status.ts`, `useBackendStatus()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (2 nodes): `input.tsx`, `Input()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 25`** (2 nodes): `label.tsx`, `cn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (2 nodes): `Collapsible()`, `collapsible.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 27`** (2 nodes): `cn()`, `button.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 28`** (2 nodes): `separator.tsx`, `cn()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (2 nodes): `score-bar.tsx`, `ScoreBar()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (2 nodes): `handleCopy()`, `cover-letter-section.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 31`** (2 nodes): `resume-dropzone.tsx`, `ResumeDropzone()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (2 nodes): `status-pill.tsx`, `StatusPill()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (1 nodes): `smoke.mjs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 43`** (1 nodes): `report-card.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (1 nodes): `results-panel.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (1 nodes): `nav-rail.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (1 nodes): `tablesort.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `GET()` connect `Community 2` to `Community 0`, `Community 9`, `Community 10`, `Community 12`?**
  _High betweenness centrality (0.161) - this node is a cross-community bridge._
- **Why does `JobReport` connect `Community 1` to `Community 2`, `Community 3`, `Community 5`, `Community 6`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J` connect `Community 1` to `Community 0`, `Community 5`?**
