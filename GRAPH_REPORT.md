...
## Corpus Check
- 43 files · ~68,108 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 412 nodes · 815 edges · 20 communities detected
- Extraction: 58% EXTRACTED · 42% INFERRED · 0% AMBIGUOUS · INFERRED: 339 edges (avg confidence: 0.64)
...
- `File Name: test_evidence_grounding.py Author: Senthilnathan Karuppaiah Date: 11-` --uses--> `JobReport`  [INFERRED]
  /home/runner/work/agent-job-matcher/agent-job-matcher/backend/tests/live/test_evidence_grounding.py → /home/runner/work/agent-job-matcher/agent-job-matcher/backend/job_matcher/schemas.py

## Communities

### Community 0 - "Community 0"
Cohesion: 0.06
Nodes (42): add_span_attributes(), _allowlisted(), current_span(), Test/advanced hook: replace the sink fan-out explicitly., The span active in this execution context, if any., Open a span nested under the context's active span (or a new trace root)., Open the per-invocation root span; the run id becomes the trace id.      When al, Attach attributes to the active span from inside a decorated call.      The sanc (+34 more)

### Community 1 - "Community 1"
Cohesion: 0.09
Nodes (36): One skill from the JD with an evidence-grounded matched verdict., SkillMatch, _count_matched(), js_round(), match_band_for(), File Name: scoring.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descrip, Map a total score (0-100) to its band; boundaries inclusive on the lower edge., Deterministic recommendation text keyed off the match band — never an LLM output (+28 more)

### Community 2 - "Community 2"
Cohesion: 0.1
Nodes (37): BaseModel, extract_jsonresume(), extract_resume_text(), File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J, Stable core entry point — lazy import so `import job_matcher` stays light., Stable core re-export of resume extraction (lazy import)., Stable core re-export of JSON Resume extraction (lazy import, async)., run_analysis() (+29 more)
...
Cohesion: 0.12
Nodes (24): _cover_letter_text(), _job_task(), _now(), _persist(), File Name: pipeline.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descri, CLI-mode persistence of the full run folder (API mode never calls this)., Run the full use case. out_dir given → CLI persistence; None → payload only., What every surface gets back from a run. (+16 more)

### Community 8 - "Community 8"
Cohesion: 0.13
Nodes (22): configure(), Resolve the sink fan-out from env, once at startup (idempotent).      The JSON-l, _build_exporters(), build_otel_sink(), OtelBridgeSink, otlp_env_configured(), File Name: observability/otel_bridge.py Author: Senthilnathan Karuppaiah Date: 1, True when any OTLP-shaped backend's activation env is present. (+14 more)
...
## Knowledge Gaps
- **55 isolated node(s):** `Return the module key for a given file path, or None if unrecognised.`, `File Name: app.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Description`, `LLM-2: pydantic-ai agent with the MCP server as its only toolset.`, `The one LLM-2 invocation per chat turn; wrapped so tests can stub it.`, `Stream a chat answer over SSE (ctms contract: content / done / error).` (+50 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 15`** (2 nodes): `main()`, `export-openapi.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 16`** (2 nodes): `index.js`, `callBackend()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 17`** (1 nodes): `smoke.mjs`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `JobReport` connect `Community 2` to `Community 1`, `Community 3`, `Community 7`, `Community 9`, `Community 12`?**
  _High betweenness centrality (0.183) - this node is a cross-community bridge._
- **Why does `run_analysis()` connect `Community 7` to `Community 0`, `Community 2`, `Community 3`, `Community 5`, `Community 9`, `Community 10`?**
  _High betweenness centrality (0.181) - this node is a cross-community bridge._
- **Why does `File Name: observability/__init__.py Author: Senthilnathan Karuppaiah Date: 11-J` connect `Community 2` to `Community 0`, `Community 1`, `Community 7`?**
  _High betweenness centrality (0.166) - this node is a cross-community bridge._
- **Are the 33 inferred relationships involving `JobReport` (e.g. with `File Name: test_pipeline_offline.py Author: Senthilnathan Karuppaiah Date: 11-JU` and `File Name: test_api.py Author: Senthilnathan Karuppaiah Date: 11-JUL-2026 Descri`) actually correct?**
  _`JobReport` has 33 INFERRED edges - model-reasoned connections that need verification._
